...
export const name = "setup2_move_to_sample";
...
export function MoveToSampleForm({
  className,
  onSubmitSuccess,
  row,
  col,
  ...
  row?: z.infer<typeof schema.shape.row>;
  col?: z.infer<typeof schema.shape.col>;
}) {
  const { add } = useQueue();
  const form = useForm({
    resolver: zodResolver(schema),
    defaultValues: {
      col: col ?? cardColumns[0],
      row: row ?? cardRows[0],
    },
  });

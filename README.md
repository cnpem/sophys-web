...
- [qua-ui](./apps/qua-ui/README.md): A web application for the QUATI beamline at LNLS.
...
> [!WARNING]
> In the Install instructions for the package in the npm registry, you will see only the suggestion to use the `npm` _package manager_, but this project requires `pnpm` as the package manager, so you will need - among other things - to replace the `npm` for `pnpm` in the install command.
